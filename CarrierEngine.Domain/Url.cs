...
public static class Url
{
    ...
    public static string Combine(string baseOrFirst, object? query, params string[] moreParts)
    {
        var allParts = new[] { baseOrFirst }.Concat(moreParts ?? Array.Empty<string>()).ToArray();
    ...
    private static List<KeyValuePair<string, string>> ToQueryPairs(object obj)
    {
        ...
        switch (obj)
        {
            case IEnumerable<KeyValuePair<string, object?>> kvs:
                ...
                foreach (var v in ExpandValue(kv.Value))
            ...
            case IDictionary dict:
                foreach (DictionaryEntry de in dict)
                {
                    ...
                    foreach (var v in ExpandValue(de.Value))
                        pairs.Add(new KeyValuePair<string, string>(key, v));
                }
                break;
            ...
            default:
                ...
                foreach (var p in props)
                {
                    ...
                    foreach (var v in ExpandValue(value))
                        pairs.Add(new KeyValuePair<string, string>(key, v));
                }
                break;
        }
    ...
    private static List<string> ExpandValue(object? value)
    {
        ...
        if (value is null) { result.Add(""); return result; }
        if (value is string s) { result.Add(s); return result; }
        if (value is byte[] bytes) { result.Add(Convert.ToBase64String(bytes)); return result; }
        if (value is DateTime dt) { result.Add(dt.ToUniversalTime().ToString("o")); return result; }
        if (value is DateTimeOffset dto) { result.Add(dto.ToUniversalTime().ToString("o")); return result; }
