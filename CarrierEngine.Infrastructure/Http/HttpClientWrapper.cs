...
public class HttpClientWrapper : IHttpClientWrapper
{
    ...
    public async Task SubmitLogs(int banyanLoadId, RequestResponseType requestResponseType, CancellationToken ct)
    {
    ...
    private async Task<ApiResult<T?>> SendAndReceiveAsync<T>(HttpMethod method, string url, object? content = null, CancellationToken cancellationToken = default)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            return ApiResult<T?>.Failure(response.StatusCode, responseBody, response.ReasonPhrase); // or throw new HttpRequestException($"Unexpected status: {response.StatusCode}");
        }

        // If there’s no content, return default instead of failing deserialization
        if (string.IsNullOrWhiteSpace(responseBody))
            return ApiResult<T?>.Success(default, response.StatusCode, responseBody);
        ...
        try
        {
            ...
            return ApiResult<T?>.Success(returnO, response.StatusCode, responseBody);
        }
        catch (JsonException ex)
        {
            return ApiResult<T?>.Failure(response.StatusCode, responseBody, ex.Message);
        }
    }
